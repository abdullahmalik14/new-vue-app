...
describe('resolveRouteAssetPreloads (Phase D §18)', () => {
  ...
  it('throws for unknown assetPreloadRef keys', () => {
    expect(() =>
      resolveRouteAssetPreloads([{ slug: '/bad', assetPreloadRef: 'missing' }], sharedAssetPreloads),
    ).toThrow(/Unknown assetPreloadRef "missing"/);
  });

  it('returns routes unchanged when no assetPreloadRef is present', () => {
    const routes = [{ slug: '/shop', section: 'shop' }];

    const resolved = resolveRouteAssetPreloads(routes, sharedAssetPreloads);

    expect(resolved[0]).toEqual(routes[0]);
  });
});
