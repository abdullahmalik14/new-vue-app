/**
 * validateRouteAssetPreloadFlags.js — Phase D (route test plan §31).
 */
...
describe('validateRouteAssetPreloadFlags (Phase D §31)', () => {
  const assetMap = {
    production: {
  ...
  it('reports missing flags with route slug context', () => {
    ...
    expect(result.errors[0]).toContain('dashboard.typo.flag');
  });

  it('rejects invalid assetPreload entry shape and priority', () => {
    const routes = [
      {
        slug: '/dashboard/overview',
        assetPreload: [
  ...
  it('validates assetPreloadRef keys against shared catalog', () => {
    const routes = [
      {
        slug: '/shop',
    ...
    expect(result.valid).toBe(false);
    expect(result.errors[0]).toContain('missingCatalogKey');
  });
});

describe('validateSharedCatalogAssetPreloadFlags (Phase D §31)', () => {
  const assetMap = {
    production: {
      'dashboard.logo': 'https://cdn.example.com/logo.webp',
    },
  ...
  it('reports missing flags in shared preload catalog arrays', () => {
    ...
    expect(errors[0]).toContain('dashboard.typo');
  });
});
