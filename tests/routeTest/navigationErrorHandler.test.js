/**
 * navigationErrorHandler.js — Phase E (route test plan §27).
 */
...
describe('navigationErrorHandler (Phase E §27)', () => {
  it('detects ChunkLoadError by name', () => {
    expect(isChunkLoadNavigationError(new Error('x'))).toBe(false);
    expect(
      isChunkLoadNavigationError(Object.assign(new Error('fail'), { name: 'ChunkLoadError' })),
    ).toBe(true);
  });
  ...
  it('detects webpack and vite chunk messages', () => {
    ...
    expect(
      isChunkLoadNavigationError(
        new Error('Failed to fetch dynamically imported module: http://localhost/x.js'),
      ),
    ).toBe(true);
  ...
  it('isOnNavigationErrorRoute returns false for non-error routes', () => {
    const router = {
      currentRoute: {
        value: {
          path: '/dashboard',
    ...
    expect(isOnNavigationErrorRoute(router)).toBe(false);
  });
});
