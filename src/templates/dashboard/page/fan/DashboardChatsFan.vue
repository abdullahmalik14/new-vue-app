...
const fetchMoreActiveMessages = async () => {
  ...
  if (res.ok) {
    if (res.data.items && res.data.items.length > 0) {
      chatStore.prependMessagesAction({ chatId: cid, items: res.data.items, pagingState: res.data.pagingState })
    }
...
const fetchAllChatsFromDB = async () => {
  ...
  for (const chatId of allChatIds) {
    ...
    if (res.ok && res.data.items && res.data.items.length > 0) {
      chatStore.prependMessagesAction({ chatId, items: res.data.items, pagingState: res.data.pagingState })
