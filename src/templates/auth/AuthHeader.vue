<template>
  ...
  <div class="flex justify-between items-start w-full">
    ...
    <div class="flex flex-col w-1/2 gap-8 sm:gap-6">
      <img class="w-8 h-8 opacity-70" :src="globeIcon || FALLBACK_GLOBE_ICON" :alt="$t('auth.header.globeAlt')" />
    </div>

    <!-- Logo -->
    <a href="/">
      <img :src="logoUrl || FALLBACK_LOGO" class="w-16 h-16" :alt="$t('auth.header.logoAlt')" />
    </a>
  </div>
</template>
