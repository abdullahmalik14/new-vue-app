<template>
  ...
  <div class="flex flex-col w-full relative gap-6 z-[5]">
    <div class="flex flex-col w-full gap-6">
      ...
      <BaseHeading :text="t('auth.reset.title')" tag="h2" theme="AuthHeading" />

      <BaseParagraph :text="t('auth.reset.subtitleConfirm')" font-size="text-base" font-weight="font-medium"
        font-color="text-white" />

      <form @submit.prevent="handleResetPasswordSubmit" class="flex flex-col gap-8">
        <!-- input-wrapper (email) -->
        ...
        <!-- input-wrapper (code) -->
        <AuthTextInput :model-value="code" @update:model-value="handleCodeInput"
        ...
        <AuthTextInput :model-value="newPassword" @update:model-value="handleNewPasswordInput"
        ...
        <!-- submit button -->
        <DashboardPrimaryButton :text="buttonText" variant="authPink" size="lg" :disabled="!isCognitoScriptReady"
          type="submit" />
      </form>
    </div>
  </div>
...
async function handleResetPasswordSubmit() {
  ...
  if (!validationResult || !validationResult.isValid) {
    ...
    if (firstInvalidField) {
      ...
      if (firstInvalidFieldState?.element) {
        ...
        const fieldErrorMessage = firstError?.message || t('auth.validation.emailRequired')
  ...
  try {
    ...
    errorMessage.value = err.message || t('auth.reset.error')
  }
}
</script>
