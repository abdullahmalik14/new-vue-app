    ...
    <div class="relative z-20 flex flex-col items-center justify-center min-h-screen gap-6 px-6">
      ...
      <div v-else-if="error" class="flex flex-col items-center gap-4 w-full max-w-md">
        ...
        <p class="text-white/70 text-sm text-center">
          {{ t('auth.oauth.telegram.tryAgain') }}
        </p>
      </div>
      ...
      <div v-else class="flex flex-col w-full max-w-md gap-6">
        <!-- Heading -->
        <BaseHeading :text="t('auth.oauth.telegram.heading')" tag="h2" theme="AuthHeading" />
...
const statusMessage = ref(t("auth.oauth.telegram.loading"));
const error = ref(null);

const state = String(route.query.state || "");

const widgetHelpText = computed(() => {
  return t("auth.oauth.telegram.widgetHelp");
});
...
function onTelegramAuth(user) {
  try {
    log("TelegramAuthPage.vue", "onTelegramAuth", "success", "Telegram widget returned user", {
      hasId: !!user?.id,
      hasHash: !!user?.hash,
      hasUsername: !!user?.username
    });

    statusMessage.value = t("auth.oauth.telegram.sending");
    isLoading.value = true;

    const ok = postToOpener("telegram-auth-success", { user });
    if (!ok) {
      error.value = t("auth.oauth.telegram.parentUnavailable");
    ...
    const ackListener = (event) => {
      ...
      if (data && data.type === "telegram-auth-ack" && data.state === state) {
        acknowledged = true;
    ...
    error.value = e?.message || t("auth.oauth.telegram.genericError");
    postToOpener("telegram-auth-error", { error: error.value });
  }
}
...
function injectTelegramWidget() {
  ...
  if (!botUsername) {
    error.value = t("auth.oauth.telegram.missingBotUsername");
    return;
  }
  ...
  script.onerror = () => {
    error.value = t("auth.oauth.telegram.widgetLoadFailed");
  };
...
onMounted(() => {
  log("TelegramAuthPage.vue", "onMounted", "start", "Mounting Telegram callback page", {
    hasState: !!state
  });
