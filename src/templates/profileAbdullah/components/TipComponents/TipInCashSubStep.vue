...
const emit = defineEmits(['update:cashAmount', 'update:isAnonymous', 'handleCashTip']);

const localCashAmount = computed({
    get: () => props.cashAmount,
    set: (value) => emit('update:cashAmount', value)
});
...
<template>
    ...
    <div class="px-2 pt-4 pb-6 md:px-4 md:pt-4 lg:px-6 lg:pt-6 xl:p-6"
        ...
        <div class="flex flex-col gap-6 md:gap-4">
            <p
                ...
                {{ showSuccess ? 'Tip again?' : 'How much would you like to tip?' }}</p>
            <div class="flex flex-col gap-2">
                <div
                    class="flex justify-between items-end gap-2 border-b border-b-border-input dark:border-b-border-input-dark">
                    <div class="flex items-baseline gap-2">
                        <span class="text-lg font-semibold text-text-primary dark:text-text-dark-primary">USD$</span>
                        <BaseInput type="number" v-model="localCashAmount" wrapperClass="" :show-controls="false"
                            inputClass="token-input w-full border-none bg-transparent outline-none text-text-primary dark:text-text-dark-primary text-[2.5rem] font-semibold leading-[3.75rem]" />
                    </div>
                </div>
                <p class="text-xs leading-normal text-text-primary dark:text-text-dark-primary">
    ...
    <div class="token-bottom-container px-2 flex flex-col gap-4 md:px-4 lg:px-6"
        ...
        <div class="flex items-center gap-2 p-2 rounded bg-bg-form dark:bg-bg-dark-form">
            ...
            <div class="flex-1 flex items-center gap-2 min-w-0">
                <template v-if="!isAnonymous">
                    ...
                    <div class="flex flex-col gap-0.5 min-w-0">
                        <span
                            class="text-xs font-semibold leading-[1.125rem] text-text-primary dark:text-text-dark-muted truncate">Fun
                            Guy</span>
                    </div>
                </template>
                <template v-else>
                    ...
                    <div class="flex flex-col gap-0.5 min-w-0">
                        <span
                            class="text-xs font-semibold leading-[1.125rem] text-text-primary dark:text-text-dark-muted truncate">Anonymous
                            User</span>
                        <span
                            class="text-xs font-medium leading-[1.125rem] text-[#667085] dark:text-[#9e9589] truncate">You
            ...
            <!-- status -->
            <div v-if="cashAmount > 0"
                class="bg-gradient-to-r from-[#FF58B2] to-[#4C57FF] w-max dark:bg-gradient-to-r dark:from-[#970051] dark:to-[#000a9f] flex items-center w-max ml-auto rounded-[0.1875rem]">
                <div class="flex items-center gap-[0.1875rem] py-0.5 px-1">
                    <span
                        class="text-base text-white font-semibold leading-[1.125rem] dark:text-[#e8e6e3] whitespace-nowrap">-USD$
                        {{ cashAmount }}</span>
                </div>
                <div class="bg-[#000000D9] p-1 flex justify-center items-center">
                    <span
                        class="text-sm font-medium leading-5 bg-clip-text text-transparent bg-[linear-gradient(90deg,rgba(255,88,178,0.7)_0%,rgba(76,87,255,0.7)_100%),linear-gradient(0deg,rgba(255,255,255,0.5),rgba(255,255,255,0.5))] dark:bg-[linear-gradient(90deg,rgba(151,0,81,0.7)_0%,rgba(0,10,159,0.7)_100%),linear-gradient(0deg,rgba(24,26,27,0.5),rgba(24,26,27,0.5))]">Pending</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <label class="relative inline-block h-5 w-9 cursor-pointer">
                <input type="checkbox" class="peer h-0 w-0 opacity-0" id="cash-switch" v-model="localIsAnonymous" />
                <span
            ...
            <!-- switch-label -->
            <label for="cash-switch" class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-2 p-2 rounded bg-bg-form dark:bg-bg-dark-form">
            ...
            <div class="flex-1 flex items-center gap-2">
                ...
                <div class="flex flex-col gap-0.5 min-w-0">
                    <span
                        class="text-xs font-semibold leading-[1.125rem] text-text-primary dark:text-text-dark-muted truncate">Princess
                        Carrot Pop</span>
                </div>
            </div>
            <!-- status -->
            <div v-if="cashAmount > 0"
                ...
                <div class="flex items-center gap-[0.1875rem] py-0.5 px-1">
                    <span
                        class="text-base text-[#182230] font-semibold leading-6 dark:text-[#d1cdc7] whitespace-nowrap">+USD$
                        {{ cashAmount }}</span>
                </div>
    ...
    <!-- button -->
    <div class="flex justify-end items-end pt-6 mt-auto z-[100000] sm:z-auto pointer-events-none">
        <div v-if="cashAmount > 0" class="h-11 md:h-14 shadow-[0px_0px_16px_0px_#FFFFFF80] pointer-events-auto">
            <button @click="$emit('handleCashTip')"
                ...
                <div class="flex items-center gap-2">
                    <span
                        class="text-base text-black font-medium leading-6 group-hover:text-[#07f468] transition-all duration-200 whitespace-nowrap">Pay
                        USD$ {{ cashAmount }}</span>
                </div>
