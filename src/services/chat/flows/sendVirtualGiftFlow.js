...
export async function sendVirtualGiftFlow({ payload, context, api }) {
  ...
  try {
    const response = await api.post(`${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/gift`, { giftData }, buildFlowRequestOptions(context));
