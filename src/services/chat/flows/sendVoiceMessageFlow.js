...
export async function sendVoiceMessageFlow({ payload, context, api }) {
  ...
  if (!chatId || !mediaUrl) {
    return fail({
      code: "SEND_VOICE_MESSAGE_MISSING_FIELDS",
      message: "chatId and mediaUrl are required.",
    });
  }

  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/voice`,
      { mediaUrl },
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "SEND_VOICE_MESSAGE_FAILED",
          message: response?.error || "Failed to send voice message.",
        },
        { flow: "chat.sendVoiceMessage", status },
      );
    }
