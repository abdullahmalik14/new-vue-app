...
export async function createGroupChatFlow({ payload, context, api }) {
  ...
  if (!payload?.createdBy || !payload?.participants) {
    return fail({
      code: "CREATE_GROUP_CHAT_MISSING_FIELDS",
      message: "createdBy and participants are required.",
    });
  }
  ...
  try {
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "CREATE_GROUP_CHAT_FAILED",
          message: response?.error || "Failed to create group chat.",
        },
        { flow: "chat.createGroupChat", status },
      );
    }
