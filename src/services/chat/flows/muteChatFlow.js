...
export async function muteChatFlow({ payload, context, api }) {
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/mute`,
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "MUTE_CHAT_FAILED",
          message: response?.error || "Failed to mute chat.",
        },
        { flow: "chat.muteChat", status },
      );
    }

    return ok(
      { muted: response?.muted, until: response?.until },
      { flow: "chat.muteChat", status },
    );
  } catch (error) {
