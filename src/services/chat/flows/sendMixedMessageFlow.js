...
export async function sendMixedMessageFlow({ payload, context, api }) {
  ...
  if (!chatId || !Array.isArray(elements)) {
    return fail({
      code: "SEND_MIXED_MESSAGE_MISSING_FIELDS",
      message: "chatId and elements (array) are required.",
    });
  }

  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/mixed`,
      { elements, senderId },
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "SEND_MIXED_MESSAGE_FAILED",
          message: response?.error || "Failed to send mixed message.",
        },
        { flow: "chat.sendMixedMessage", status },
      );
    }
