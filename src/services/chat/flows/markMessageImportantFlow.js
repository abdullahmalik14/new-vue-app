...
export async function markMessageImportantFlow({ payload, context, api }) {
  ...
  try {
    const response = await api.post(`${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/important`, { important }, buildFlowRequestOptions(context));
