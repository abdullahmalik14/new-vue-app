...
export async function markMessageImportantFlow({ payload, context, api }) {
  ...
  if (!chatId || !messageId) {
    return fail({
      code: "MARK_MESSAGE_IMPORTANT_MISSING_FIELDS",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/important`,
      { important },
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "MARK_MESSAGE_IMPORTANT_FAILED",
          message: response?.error || "Failed to mark message as important.",
        },
        { flow: "chat.markMessageImportant", status },
      );
    }
