...
export async function storeChatMessageFlow({ payload, context, api }) {
  ...
  if (!chatId) {
    return fail({
      code: "STORE_CHAT_MESSAGE_MISSING_CHAT_ID",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/store`,
      messagePayload,
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "STORE_CHAT_MESSAGE_FAILED",
          message: response?.error || "Failed to store message.",
        },
        { flow: "chat.storeChatMessage", status },
      );
    }
