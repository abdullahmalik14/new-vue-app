...
export async function getChatStatsFlow({ payload, context, api }) {
  ...
  if (!chatId) {
    return fail({
      code: "GET_CHAT_STATS_MISSING_CHAT_ID",
  ...
  try {
    const response = await api.get(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/stats`,
      ...
      return fail(
        {
          ...
          message: response?.error || "Failed to fetch chat stats.",
        },
        { flow: "chat.getChatStats", status },
      );
    }
