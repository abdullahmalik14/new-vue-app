...
export async function archiveChatFlow({ payload, context, api }) {
  ...
  if (!chatId || !userId) {
    return fail({
      code: "ARCHIVE_CHAT_MISSING_FIELDS",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/archive`,
      { userId },
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "ARCHIVE_CHAT_FAILED",
          message: response?.error || "Failed to archive chat.",
        },
        { flow: "chat.archiveChat", status },
      );
    }
