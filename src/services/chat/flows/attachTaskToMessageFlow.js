...
export async function attachTaskToMessageFlow({ payload, context, api }) {
  ...
  try {
    const response = await api.post(`${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/task`, { taskId }, buildFlowRequestOptions(context));
