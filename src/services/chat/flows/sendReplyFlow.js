...
export async function sendReplyFlow({ payload, context, api }) {
  ...
  if (!chatId || !messageId) {
    return fail({
      code: "SEND_REPLY_MISSING_FIELDS",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/replies`,
      { contentType, text, senderId },
      ...
      return fail(
        {
          ...
          message: response?.error || "Failed to send reply.",
        },
        { flow: "chat.sendReply", status },
      );
    }
