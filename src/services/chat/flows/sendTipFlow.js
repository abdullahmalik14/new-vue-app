...
export async function sendTipFlow({ payload, context, api }) {
  ...
  if (!chatId || !messageId || !amount) {
    return fail({
      code: "SEND_TIP_MISSING_FIELDS",
      message: "chatId, messageId, and amount are required.",
    });
  }

  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/tip`,
      { amount, currency },
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "SEND_TIP_FAILED",
          message: response?.error || "Failed to send tip.",
        },
        { flow: "chat.sendTip", status },
      );
    }
