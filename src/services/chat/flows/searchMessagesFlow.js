...
export async function searchMessagesFlow({ payload, context, api }) {
  ...
  if (!chatId) {
    return fail({
      code: "SEARCH_MESSAGES_MISSING_CHAT_ID",
  ...
  try {
    ...
    const response = await api.get(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/search`,
      { ...buildFlowRequestOptions(context), params: query },
    );
      ...
      return fail(
        {
          ...
          message: response?.error || "Failed to search messages.",
        },
        { flow: "chat.searchMessages", status },
      );
    }
