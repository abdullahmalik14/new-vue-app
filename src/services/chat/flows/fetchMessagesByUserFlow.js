...
export async function fetchMessagesByUserFlow({ payload, context, api }) {
  ...
  if (!chatId || !userId) {
    return fail({
      code: "FETCH_MESSAGES_BY_USER_MISSING_FIELDS",
  ...
  try {
    const response = await api.get(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/by-user/${encodeURIComponent(userId)}`,
      {
        ...buildFlowRequestOptions(context),
        params: { since, until, limit, sort },
      },
    );
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "FETCH_MESSAGES_BY_USER_FAILED",
          message: response?.error || "Failed to fetch messages by user.",
        },
        { flow: "chat.fetchMessagesByUser", status },
      );
    }
