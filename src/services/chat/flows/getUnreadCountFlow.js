...
export async function getUnreadCountFlow({ payload, context, api }) {
  ...
  if (!chatId || !userId) {
    return fail({
      code: "GET_UNREAD_COUNT_MISSING_FIELDS",
  ...
  try {
    const response = await api.get(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/unread`,
      { ...buildFlowRequestOptions(context), params: { userId } },
    );
      ...
      return fail(
        {
          ...
          message: response?.error || "Failed to fetch unread count.",
        },
        { flow: "chat.getUnreadCount", status },
      );
    }
