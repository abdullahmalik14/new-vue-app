...
export async function deleteChatHistoryFlow({ payload, context, api }) {
  ...
  if (!chatId) {
    return fail({
      code: "DELETE_CHAT_HISTORY_MISSING_CHAT_ID",
  ...
  try {
    const response = await api.delete(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/history`,
      { ...buildFlowRequestOptions(context), ...{ hardDelete, range } },
    );
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "DELETE_CHAT_HISTORY_FAILED",
          message: response?.error || "Failed to delete chat history.",
        },
        { flow: "chat.deleteChatHistory", status },
      );
    }
