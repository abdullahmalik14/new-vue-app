...
export async function updateChatMetadataFlow({ payload, context, api }) {
  ...
  if (!chatId) {
    return fail({
      code: "UPDATE_CHAT_METADATA_MISSING_CHAT_ID",
  ...
  try {
    const response = await api.patch(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/metadata`,
      metadata,
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "UPDATE_CHAT_METADATA_FAILED",
          message: response?.error || "Failed to update chat metadata.",
        },
        { flow: "chat.updateChatMetadata", status },
      );
    }
