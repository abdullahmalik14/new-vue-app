...
export async function removeChatParticipantFlow({ payload, context, api }) {
  ...
  try {
    const response = await api.delete(`${baseUrl}/chats/${encodeURIComponent(chatId)}/participants/${encodeURIComponent(userId)}`, { ...buildFlowRequestOptions(context), ...{ by, reason } });
