...
export async function getMessageStatsFlow({ payload, context, api }) {
  ...
  if (!chatId || !messageId) {
    return fail({
      code: "GET_MESSAGE_STATS_MISSING_FIELDS",
  ...
  try {
    const response = await api.get(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/stats`,
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "GET_MESSAGE_STATS_FAILED",
          message: response?.error || "Failed to fetch message stats.",
        },
