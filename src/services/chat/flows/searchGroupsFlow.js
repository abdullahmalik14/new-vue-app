...
export async function searchGroupsFlow({ payload, context, api }) {
  ...
  if (!q) {
    return fail({
      code: "SEARCH_GROUPS_MISSING_QUERY",
      message: "q (search query) is required.",
    });
  }

  try {
    const response = await api.get(`${baseUrl}/chats/groups/search`, {
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "SEARCH_GROUPS_FAILED",
          message: response?.error || "Failed to search groups.",
        },
        { flow: "chat.searchGroups", status },
      );
    }
