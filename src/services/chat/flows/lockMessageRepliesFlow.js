...
export async function lockMessageRepliesFlow({ payload, context, api }) {
  ...
  if (!chatId || !messageId) {
    return fail({
      code: "LOCK_MESSAGE_REPLIES_MISSING_FIELDS",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/lock`,
      { lock },
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "LOCK_MESSAGE_REPLIES_FAILED",
          message: response?.error || "Failed to lock message replies.",
        },
        { flow: "chat.lockMessageReplies", status },
      );
    }
