...
export async function startChatTrialFlow({ payload, context, api }) {
  ...
  if (!chatId || !userId) {
    return fail({
      code: "START_CHAT_TRIAL_MISSING_FIELDS",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/trial`,
      { userId, trialDays },
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "START_CHAT_TRIAL_FAILED",
          message: response?.error || "Failed to start chat trial.",
        },
        { flow: "chat.startChatTrial", status },
      );
    }
