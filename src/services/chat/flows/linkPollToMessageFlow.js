...
export async function linkPollToMessageFlow({ payload, context, api }) {
  ...
  try {
    const response = await api.post(`${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/poll`, { pollId }, buildFlowRequestOptions(context));
