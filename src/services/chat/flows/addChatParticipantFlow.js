...
export async function addChatParticipantFlow({ payload, context, api }) {
  ...
  if (!chatId || (!userId && !isMulti)) {
    return fail({
      code: "ADD_CHAT_PARTICIPANT_MISSING_FIELDS",
      message: "chatId and userId or userIds are required.",
    });
  }

  try {
    const body = isMulti
      ? { userIds, role, invitedBy }
      : { userId, role, invitedBy };
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/participants`,
      body,
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "ADD_CHAT_PARTICIPANT_FAILED",
          message: response?.error || "Failed to add participant.",
        },
        { flow: "chat.addChatParticipant", status },
      );
    }

    if (response?.results) {
      return ok(
        { chatId, results: response.results },
        { flow: "chat.addChatParticipant", status },
      );
    }
    return ok(
