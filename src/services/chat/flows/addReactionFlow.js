...
export async function addReactionFlow({ payload, context, api }) {
  ...
  if (!chatId || !messageId || !emoji) {
    return fail({
      code: "ADD_REACTION_MISSING_FIELDS",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/reactions`,
      { emoji, count },
      ...
      return fail(
        {
          ...
          message: response?.error || "Failed to add reaction.",
        },
        { flow: "chat.addReaction", status },
      );
    }
