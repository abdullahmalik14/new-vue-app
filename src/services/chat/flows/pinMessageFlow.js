...
export async function pinMessageFlow({ payload, context, api }) {
  ...
  if (!chatId || !messageId) {
    return fail({
      code: "PIN_MESSAGE_MISSING_FIELDS",
  ...
  try {
    const response = await api.post(
      `${baseUrl}/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}/pin`,
      { pin },
      ...
      return fail(
        {
          ...
          message: response?.error || "Failed to pin message.",
        },
        { flow: "chat.pinMessage", status },
      );
    }
