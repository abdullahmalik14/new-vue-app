...
export async function updateBookingRequestMessageFlow({
  payload,
  context,
  api,
}) {
  const baseUrl = getChatApiBaseUrl(context);
  const { chatId, messageId, action, slotDate, eventTitle, text, meta } =
    payload;

  if (!chatId || !messageId) {
    return fail({
      code: "UPDATE_BOOKING_REQUEST_MISSING_FIELDS",
  ...
  if (
    action &&
    !["pending", "accepted", "declined", "counter_offer"].includes(action)
  ) {
    return fail({
  ...
  try {
    ...
    if (response?.ok === false) {
      return fail(
        {
          code: "UPDATE_BOOKING_REQUEST_FAILED",
          message:
            response?.error || "Failed to update booking request message.",
        },
