...
const EXTERNAL_API_BASE_URL =
  import.meta.env.VITE_WEB_BASE_URL + "/wp-json/api";
...
function mapPrimaryResponse(type, response = {}, payload = {}) {
  ...
  return {
    type,
    source:
      type === "subscription" ? "subscriptions/plans/list" : `${type}/list`,
    results,
    totalCount,
    count: toNumberOr(payload?.count, 20),
    offset: Math.max(0, toNumberOr(payload?.offset, 0)),
...
async function fetchSubscriptionFallback({ payload, context, api }) {
  const response = await api.get(
    `${EXTERNAL_API_BASE_URL}/users/active-tiers`,
    {
      ...buildFlowRequestOptions(context),
      params: {
...
export async function fetchSpendingRequirementItemsFlow({
  payload,
  context,
  api,
}) {
  ...
  try {
    const primaryResult = await fetchPrimaryCollection({
      type,
      payload,
      context,
      api,
    ...
    if (
      type === "subscription" &&
      String(primaryResult?.error?.code || "") === "HTTP_404"
    ) {
    ...
    if (
      type === "subscription" &&
      String(error?.error?.code || error?.code || "") === "HTTP_404"
    ) {
      try {
        return await fetchSubscriptionFallback({ payload, context, api });
      } catch (fallbackError) {
