...
const renderHeaderNav = () => {
  ...
  const itemsToUse = resolvedMenuItems.value.length > 0 ? resolvedMenuItems.value : menuItems;
  if (!menuEl || !itemsToUse.length) return;
...
onMounted(async () => {
  ...
  try {
    resolvedMenuItems.value = await resolveMenuItemsWithAssets(menuItems);
  } catch (error) {
    console.warn('[NavDropdown] Failed to resolve menu items with assets, using original menuItems', error);
...
function watchLocaleChanges() {
  ...
  if (!i18nInstance) {
    console.warn('[NavDropdown] i18n instance not available for locale watching');
    return;
  }
  ...
  const checkInterval = setInterval(async () => {
    try {
      ...
      if (currentLocale !== previousLocale) {
        console.log(`[NavDropdown] Locale changed from '${previousLocale}' to '${currentLocale}'`);
        previousLocale = currentLocale;

        // Re-resolve menu items with new translations
        try {
          resolvedMenuItems.value = await resolveMenuItemsWithAssets(menuItems);
        } catch (error) {
          console.warn('[NavDropdown] Failed to resolve menu items after locale change', error);
        }

        // Re-render the navigation with updated translations
        renderHeaderNav();
      }
    } catch (error) {
      console.warn('[NavDropdown] Error checking locale change', error);
    }
...
<template>
  <BasePopupShell :modelValue="props.modelValue" @update:modelValue="(val) => emit('update:modelValue', val)"
    :config="config">
    <div
      ...
      <div class="flex flex-col items-end self-stretch">
        <a @click="emit('update:modelValue', false)" class="flex items-center justify-center w-8 h-8 cursor-pointer">
          <img src="https://i.ibb.co/HfPJmd0W/svgviewer-png-output-40.webp" alt="svgviewer-png-output-40"
            class="w-6 h-6" />
        </a>
      </div>
  ...
  <!-- Submenu Popup -->
  <BasePopupShell v-model="showSubmenuPopup" :config="submenuPopupConfig" :is-loading="false">
    <div
      ...
      <div class="flex flex-col gap-4 w-full">
        ...
        <div class="flex items-center justify-center gap-2 w-full">
          <img v-if="selectedMenuItem?.image" :src="selectedMenuItem.image" :alt="selectedMenuItem.title"
            class="w-5 h-5" />
      ...
      <div class="flex flex-col overflow-auto w-full gap-2 py-2">
        <button v-for="child in selectedMenuItem?.children" :key="child.id" :disabled="!child.enabled"
          @click="(e) => child.enabled && handleSubmenuClick(e, child)" :class="[
