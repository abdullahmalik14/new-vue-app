<template>
  <DashboardAnalyticsTrendPopup :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)"
    ...
    :title="$t('dashboard.analytics.trends.titleContributors', 'Contributors Insight')" :logo="iconPopupLogo || ''">
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-3 p-4 rounded-sm w-full h-[22rem] relative">
        <div class="flex justify-between items-center z-10 relative">
          <h3 class="text-base font-semibold text-[#101828] dark:text-[#dbd8d3]">{{
            $t('dashboard.analytics.trends.topContributors') }}</h3>
          <div class="flex gap-1 bg-[#F9FAFB] p-1 rounded-lg border border-[#EAECF0]">
            <button class="p-1.5 rounded-md cursor-pointer transition-all focus:outline-none hover:!bg-transparent"
              :class="activeContribViewMode === 'bar' ? 'bg-white shadow-sm' : 'bg-transparent'"
              @click="setContribView('bar')">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                :stroke="activeContribViewMode === 'bar' ? '#344054' : '#98A2B3'" stroke-width="2" stroke-linecap="round">
            ...
            <button class="p-1.5 rounded-md cursor-pointer transition-all focus:outline-none hover:!bg-transparent"
              :class="activeContribViewMode === 'line' ? 'bg-white shadow-sm' : 'bg-transparent'"
              @click="setContribView('line')">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                :stroke="activeContribViewMode === 'line' ? '#344054' : '#98A2B3'" stroke-width="2" stroke-linecap="round">
        ...
        <div class="absolute top-[40px] left-0 right-0 bottom-[10px] transition-opacity duration-200"
          :class="{ 'opacity-0': isChartRendering, 'opacity-100': !isChartRendering }"
          v-show="analyticsStore.bundleLoaded && insightData?.topContributors?.length > 0">
          <div data-chart-container data-chart-id="contrib-top-bar" :hidden="activeContribViewMode !== 'bar' || undefined"
            class="absolute inset-0" :data-chart-config='getContribBarCfg("contrib-top")'>
            <div amchart data-role="chart" style="width:100%;height:100%;"></div>
          </div>
          <div data-chart-container data-chart-id="contrib-top-line" :hidden="activeContribViewMode !== 'line' || undefined"
            class="absolute inset-0" :data-chart-config='getContribLineCfg("contrib-top")'>
            <div amchart data-role="chart" style="width:100%;height:100%;"></div>
          </div>
        </div>
        <div
          ...
          <div class="flex flex-col gap-1">
            <span class="text-xs leading-6 text-light-text-secondary dark:text-dark-text-secondary">{{
              $t('dashboard.analytics.trends.noTrend') }}</span>
            <a href="#"
              class="text-[10px] leading-6 text-light-text-secondary dark:text-dark-text-secondary underline">{{
                $t('dashboard.analytics.trends.learnToEarn') }}</a>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-3 p-4 rounded-sm w-full h-[22rem] relative">
        <div class="flex justify-between items-center z-10 relative">
          <h3 class="text-base font-semibold text-[#101828] dark:text-[#dbd8d3]">{{
            $t('dashboard.analytics.trends.topFans') }}</h3>
          <div class="flex gap-1 bg-[#F9FAFB] p-1 rounded-lg border border-[#EAECF0]">
            <button class="p-1.5 rounded-md cursor-pointer transition-all focus:outline-none hover:!bg-transparent"
              :class="activeFansViewMode === 'bar' ? 'bg-white shadow-sm' : 'bg-transparent'" @click="setFansView('bar')">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                :stroke="activeFansViewMode === 'bar' ? '#344054' : '#98A2B3'" stroke-width="2" stroke-linecap="round">
                <line x1="18" y1="20" x2="18" y2="10" />
                <line x1="12" y1="20" x2="12" y2="4" />
                <line x1="6" y1="20" x2="6" y2="14" />
              </svg>
            ...
            <button class="p-1.5 rounded-md cursor-pointer transition-all focus:outline-none hover:!bg-transparent"
              :class="activeFansViewMode === 'line' ? 'bg-white shadow-sm' : 'bg-transparent'" @click="setFansView('line')">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                :stroke="activeFansViewMode === 'line' ? '#344054' : '#98A2B3'" stroke-width="2" stroke-linecap="round">
                <polyline points="22 12 18 12 15 21 9 3 6 12 2 12" />
              </svg>
            </button>
          </div>
        ...
        <div class="absolute top-[40px] left-0 right-0 bottom-[10px] transition-opacity duration-200"
          :class="{ 'opacity-0': isChartRendering, 'opacity-100': !isChartRendering }"
          v-show="analyticsStore.bundleLoaded && insightData?.topFans?.length > 0">
          <div data-chart-container data-chart-id="contrib-fans-bar" :hidden="activeFansViewMode !== 'bar' || undefined"
            class="absolute inset-0" :data-chart-config='getContribBarCfg("contrib-fans")'>
            <div amchart data-role="chart" style="width:100%;height:100%;"></div>
          </div>
          <div data-chart-container data-chart-id="contrib-fans-line" :hidden="activeFansViewMode !== 'line' || undefined"
            class="absolute inset-0" :data-chart-config='getContribLineCfg("contrib-fans")'>
            <div amchart data-role="chart" style="width:100%;height:100%;"></div>
          </div>
        </div>
        <div
      ...
      <div
        class="flex flex-col gap-3 p-4 rounded-sm  backdrop-blur-[25px] dark:bg-dark-bg-container w-full h-[22rem] relative">
        <div class="flex justify-between items-center z-10 relative">
          <h3 class="text-base font-semibold text-[#101828] dark:text-[#dbd8d3]">{{
            $t('dashboard.analytics.trends.topOrderSpenders') }}</h3>
          <div class="flex gap-1 bg-[#F9FAFB] p-1 rounded-lg border border-[#EAECF0]">
            <button class="p-1.5 rounded-md cursor-pointer transition-all focus:outline-none hover:!bg-transparent"
              :class="activeSpendersViewMode === 'bar' ? 'bg-white shadow-sm' : 'bg-transparent'"
              @click="setSpendersView('bar')">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                :stroke="activeSpendersViewMode === 'bar' ? '#344054' : '#98A2B3'" stroke-width="2" stroke-linecap="round">
                <line x1="18" y1="20" x2="18" y2="10" />
                <line x1="12" y1="20" x2="12" y2="4" />
                <line x1="6" y1="20" x2="6" y2="14" />
              </svg>
            ...
            <button class="p-1.5 rounded-md cursor-pointer transition-all focus:outline-none hover:!bg-transparent"
              :class="activeSpendersViewMode === 'line' ? 'bg-white shadow-sm' : 'bg-transparent'"
              @click="setSpendersView('line')">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                :stroke="activeSpendersViewMode === 'line' ? '#344054' : '#98A2B3'" stroke-width="2" stroke-linecap="round">
                <polyline points="22 12 18 12 15 21 9 3 6 12 2 12" />
              </svg>
            </button>
          </div>
        ...
        <div class="absolute top-[40px] left-0 right-0 bottom-[10px]"
          v-show="analyticsStore.bundleLoaded && insightData?.topOrderSpenders?.length > 0">
          <div data-chart-container data-chart-id="contrib-spenders-bar"
            :hidden="activeSpendersViewMode !== 'bar' || undefined" class="absolute inset-0"
            :data-chart-config='getContribBarCfg("contrib-spenders")'>
            <div amchart data-role="chart" style="width:100%;height:100%;"></div>
          </div>
          <div data-chart-container data-chart-id="contrib-spenders-line"
            :hidden="activeSpendersViewMode !== 'line' || undefined" class="absolute inset-0"
