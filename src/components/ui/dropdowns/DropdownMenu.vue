...
<script setup>
/**
 * DropdownMenu Component - SSR-Safe Implementation
 * 
 ...
 * - Helper functions (from dropdownLayoutUtils.js) receive DOM data via parameters:
  ...
  for (const [id, entry] of __DROPDOWN_STACK.entries()) {
    ...
    if (!entry.instance || typeof entry.instance.close !== 'function') {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn(`[DropdownMenu] Cleaned up stale stack entry for dropdown: ${id}`)
      }
    }
  }
  return cleaned
...
function __stackRegister(dropdownId, instance, layer = 'dropdown', group = null) {
  ...
  for (const [id, entry] of __DROPDOWN_STACK.entries()) {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn(`[DropdownMenu] Error closing dropdown ${id} during stack register:`, e)
      }
...
// ===== Z-INDEX RESERVATION =====
// 
// IMPORTANT: By default, DropdownMenu reserves z-index 999-1000 for dropdown overlays and dropdowns.
...
const DEFAULTS = Object.freeze({
  ...
  //
  // @see isMobile() helper in dropdownLayoutUtils.js for implementation
...
function validateConfig(cfg) {
  ...
  if (!cfg || typeof cfg !== 'object') {
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Config validation failed: config is null or not an object')
    }
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (errors.length > 0) {
        console.error('[DropdownMenu] Config validation errors:', errors)
      }
      if (warnings.length > 0) {
        console.warn('[DropdownMenu] Config validation warnings:', warnings)
      }
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Unexpected error during config validation:', error)
      console.error('[DropdownMenu] Config object:', cfg)
    }
...
const config = computed(() => {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Error in config computed, using defaults:', error)
    }
    return { ...DEFAULTS }
  }
})
...
const teleportTarget = computed(() => {
  ...
  if (typeof document !== 'undefined') {
    try {
      ...
      if (!el) {
        if (process.env.NODE_ENV !== 'production') {
          console.warn(`[DropdownMenu] Teleport target "${target}" not found, falling back to 'body'`)
        }
        resolvedTeleportTarget.value = 'body'
        return 'body'
      }
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn(`[DropdownMenu] Error querying teleport target "${target}":`, e)
      }
      resolvedTeleportTarget.value = 'body'
      return 'body'
    }
...
function sharedDocumentClick(e) {
  ...
  for (const handler of globalClickHandlers) {
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Error in document click handler:', error)
      }
    }
  }
}
...
function sharedEscapeKey(e) {
  ...
  for (const handler of globalEscapeHandlers) {
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Error in escape key handler:', error)
      }
    }
  }
}
...
function sharedWindowScroll(e) {
  ...
  for (const handler of globalScrollHandlers) {
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Error in window scroll handler:', error)
      }
    }
  }
}
...
function sharedWindowResize(e) {
  ...
  for (const handler of globalResizeHandlers) {
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Error in window resize handler:', error)
      }
    }
  }
}
  ...
  try {
    ...
    if (previousCount === 0) {
      ...
      if (body && body.style && body.style.overflow !== 'hidden') {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.warn(
            `[DropdownMenu] Body scroll was unlocked by another component while dropdown "${dropdownId}" was open. ` +
    ...
    scrollLockCount = Math.max(0, scrollLockCount - 1)
    if (process.env.NODE_ENV !== 'production') {
      console.warn('[DropdownMenu] Error preventing body scroll:', error)
    }
  }
}
...
function restoreBodyScroll() {
  ...
  try {
    ...
    if (scrollLockCount === 0) {
      ...
      if (body && body.style && body.style.overflow === 'hidden') {
        ...
        if (process.env.NODE_ENV !== 'production') {
          ...
          console.warn(
            `[DropdownMenu] Body scroll was changed by another component while locked. ` +
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.debug(
          `[DropdownMenu] Not restoring body scroll: ${scrollLockCount} dropdown(s) still have locks. ` +
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn('[DropdownMenu] Error restoring body scroll:', error)
    }
  }
}
...
function resolveAnchor() {
  try {
    ...
    if (!props.anchor) {
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Anchor prop is null or undefined. Ensure anchor is provided and mounted.')
      }
      return null
    }
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        const anchorInfo = props.anchor?.$el ? 'component (no $el)' :
          props.anchor?.value ? `ref (value: ${typeof props.anchor.value})` :
            `type: ${anchorType}`
        console.error(
          `[DropdownMenu] Anchor element not found or not mounted. ` +
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Unexpected error resolving anchor:', e)
      console.error('[DropdownMenu] Anchor prop value:', props.anchor)
    }
    return null
  }
}
...
function repositionInternal(reason = 'manual') {
  ...
  try {
    ...
    if (!anchorEl) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Cannot reposition: anchor element not available')
      }
      return
    }
    ...
    if (!dd) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.debug(
          `[DropdownMenu] Dropdown element not yet mounted, scheduling retry. ` +
      ...
      if (typeof requestAnimationFrame !== 'undefined') {
        requestAnimationFrame(() => {
          ...
          if (isOpenInternal.value && dropdownRef.value) {
            ...
            console.warn(
              `[DropdownMenu] Dropdown element still not available after retry. ` +
    ...
    if (typeof document !== 'undefined' && !document.body.contains(dd)) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.debug(
          `[DropdownMenu] Dropdown element not yet in DOM, scheduling retry. ` +
          `Reason: ${reason}.`
        )
      }
    ...
    if (isTimingError) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.debug(
          `[DropdownMenu] Transient error during reposition (timing issue), scheduling retry. ` +
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Error during reposition, closing dropdown softly:', error)
      }
...
function open(mode = config.value.trigger) {
  try {
    ...
    if (!cachedAnchorEl) {
      // Missing anchor is a critical error - log as error, not warning
      if (process.env.NODE_ENV !== 'production') {
        console.error('[DropdownMenu] Cannot open: anchor element not available')
      }
      return false
    }
    ...
    nextTick(() => {
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.error('[DropdownMenu] Error in repositionInternal during open:', error)
        }
        ...
        try {
          ...
          if (config.value.useTransitionAnimation && config.value.animation === 'slide-up') {
              ...
              requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                  try {
                    ...
                    if (process.env.NODE_ENV !== 'production') {
                      console.error('[DropdownMenu] Error in slide-up animation setup:', error)
                    }
                    isTransitioning.value = false
                  }
                })
              ...
              requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                  try {
                    ...
                    // Error boundary: catch errors in animation setup
                    if (process.env.NODE_ENV !== 'production') {
                      console.error('[DropdownMenu] Error in slide-down animation setup:', error)
                    }
                    isTransitioning.value = false
                  }
                })
              ...
              requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                  try {
                    ...
                    // Error boundary: catch errors in animation setup
                    if (process.env.NODE_ENV !== 'production') {
                      console.error('[DropdownMenu] Error in keyframe animation setup:', error)
                    }
                    isTransitioning.value = false
                  }
                })
              ...
              try {
                ...
                if (process.env.NODE_ENV !== 'production') {
                  console.error('[DropdownMenu] Error in no-animation setup:', error)
                }
                isTransitioning.value = false
              }
            }
          ...
          if (process.env.NODE_ENV !== 'production') {
            console.error('[DropdownMenu] Error in animation setup or content scroll prevention:', error)
          }
          isTransitioning.value = false
    ...
    isTransitioning.value = false
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Error during open:', error)
    }
    isOpenInternal.value = false
    return false
  }
...
function setupContentScrollPrevention() {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn('[DropdownMenu] Error setting up content scroll prevention:', error)
    }
...
function close(origin = 'manual', force = false) {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Error during close:', error)
    }
...
* 
* @param {string} name - Event name (will be prefixed with 'DropdownMenu:')
...
function dispatchDomEvent(name, detail) {
  ...
  try {
    document.dispatchEvent(new CustomEvent(`DropdownMenu:${name}`, { detail }))
  } catch (error) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn(`[DropdownMenu] Failed to dispatch DOM event "${name}":`, error)
    }
  }
}
...
function getTransitionDurationMs(element, durationMs) {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn('[DropdownMenu] Failed to read CSS transition duration, falling back to configured duration:', error)
    }
    return fallback
  }
}
...
function watchTransition(element, durationMs, onComplete) {
  ...
  timeoutId = setTimeout(() => {
    if (process.env.NODE_ENV !== 'production' && dropdownRef.value === element) {
      console.warn('[DropdownMenu] Transition timeout fallback triggered - transitionend did not fire within expected time')
    }
    complete()
  }, timeoutDuration)
...
function bindTrigger() {
  ...
  if (previousAnchorEl) {
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[DropdownMenu] Error cleaning up previous anchor listeners (anchor may be removed):', error)
      }
  ...
  if (!anchorEl) {
    // Missing anchor is a critical error - log as error, not warning
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Cannot bind trigger: anchor element not available')
    }
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn('[DropdownMenu] Error removing existing listeners from anchor:', error)
    }
  }
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.error('[DropdownMenu] Error binding trigger listeners:', error)
    }
...
function setupHover(anchorEl) {
  ...
  if (!Number.isFinite(hoverDelay) || hoverDelay < 0) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn(
        `[DropdownMenu] Invalid hoverIntentMs: "${config.value.hoverIntentMs}". ` +
...
onMounted(() => {
  ...
  nextTick(() => {
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[DropdownMenu] Error binding trigger on mount:', e)
      }
    }
  })
})
    ...
    try {
      ...
      if (isOpenInternal.value) {
        ...
        if (newAnchorEl) {
          ...
          if (process.env.NODE_ENV !== 'production') {
            console.warn('[DropdownMenu] Anchor changed to invalid element, closing dropdown')
          }
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[DropdownMenu] Error rebinding trigger after anchor change:', e)
      }
    }
  })
...
watch(() => [config.value.trigger, config.value.toggleOnTriggerClick, config.value.hoverIntentMs], () => {
  nextTick(() => {
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[DropdownMenu] Error rebinding trigger after config change:', e)
      }
    }
  })
}, { deep: false })
  ...
  if (isOpenInternal.value) {
    nextTick(() => {
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.warn('[DropdownMenu] Error updating scroll listeners after config change:', e)
        }
      }
    })
  }
