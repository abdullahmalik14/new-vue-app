/**
 * DropdownMenu Helper Functions and Theme Mapping
 * External utilities to support the DropdownMenu component
 * 
 ...
 * @example
 * // In consumer code:
 * import { CLOSE_REASONS } from './dropdownLayoutUtils.js'
 * 
...
export function getThemeClasses(themeName) {
  ...
  if (!themeName) {
    if (process.env.NODE_ENV !== 'production') {
      console.warn(
        `[DropdownMenu] Theme name is ${themeName === null ? 'null' : 'undefined'}, ` +
  ...
  if (process.env.NODE_ENV !== 'production' && !THEME_CLASS_MAP[themeName] && themeName !== 'generic') {
    console.warn(
      `[DropdownMenu] Unknown theme "${themeName}", falling back to 'generic'. ` +
    ...
    if (config.widthMode === 'parent' && config.widthParentSelector) {
      ...
      if (!referenceEl) {
        ...
        if (!warnedSelectors.has(selectorKey)) {
          ...
          if (process.env.NODE_ENV !== 'production') {
            console.warn(`[DropdownMenu] Parent selector "${config.widthParentSelector}" not found, falling back to viewport mode`)
          }
        }
      }
      ...
      if (!referenceEl) {
        ...
        if (!warnedSelectors.has(selectorKey)) {
          ...
          // Only warn in development to avoid production console noise
          if (process.env.NODE_ENV !== 'production') {
            console.warn(`[DropdownMenu] Ancestor selector "${config.widthAncestorSelector}" not found, falling back to viewport mode`)
          }
        }
      }
    }
 ...
 * It's called reactively from DropdownMenu on each reposition, so changes
 ...
 * 
 * @example
 * // Used in DropdownMenu repositionInternal()
