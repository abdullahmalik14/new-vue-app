...
export class APIHandler {
    ...
    constructor(defaults = {}) {
        ...
        // Store MockApi instance if provided (for offline mode)
        this.mockApi = defaults.mockApi || null;
        this.useMockApi = defaults.useMockApi || false;
    }
    ...
    async handleRequest(apiParams) {
        ...
        try {
            ...
            // Check if we should use MockApi (offline mode)
            let response;
            if (this.useMockApi && this.mockApi) {
                console.log("-> APIHandler: Using MockApi (offline mode)", ["endpoint = " + apiBaseUrl, "filename = api.js", "reference_code = Mo12"]);
                ...
                switch (httpMethod.toUpperCase()) {
                    case 'GET':
                        mockResponse = await this.mockApi.get(apiBaseUrl, mockOptions);
                        break;
                    case 'POST':
                    ...
                    case 'DELETE':
                        ...
                        break;
                    default:
                        throw new Error(`Method ${httpMethod} not supported by MockApi`);
                }

                // MockApi returns a Response-like object, convert to standard Response format
                response = mockResponse;
            } else {
                // Use real API via fetch
    ...
    async processResponse(response, apiParams) {
        ...
        if (response.ok) {
            console.log("-> APIHandler: Parsed Response Data", ["data = " + JSON.stringify(responseData), "filename = api.js", "reference_code = Ef34"]);

            // Dispatch a success event with the response data
            this.dispatchSuccessEvent(apiParams, response, responseData);
     ...
     */
    dispatchErrorEvent(apiParams, error) {
        // Dispatch a custom event with the response data
        document.dispatchEvent(new CustomEvent('dash-api-handler-response', {
            detail: { success: false, error: true, data: null, args: apiParams, error_message: error }
        }));
    }
}

// Expose globally for use in apiWrapper
