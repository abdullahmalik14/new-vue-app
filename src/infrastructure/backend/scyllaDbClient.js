...
const BASE_URL =
  import.meta.env.VITE_SCYLLADB_API_URL || "http://localhost:8080/api";
...
export async function linkSocialAccount(userId, provider, socialId) {
  log(
    ...
    "linkSocialAccount",
    "start",
    "Linking social account in ScyllaDB",
...
export async function unlinkSocialAccount(userId, provider) {
  log(
    ...
    "unlinkSocialAccount",
    "start",
    "Unlinking social account in ScyllaDB",
    { userId, provider },
  );
...
export async function getUserData(userId) {
  log(
    ...
    "getUserData",
    "start",
    "Fetching user data from ScyllaDB",
    { userId },
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log(
        ...
        "postTelegramMessage",
        "error",
        "Failed to post message to Telegram",
        {
          userId,
...
export async function getTwitterTokens(userId) {
  ...
  try {
    ...
    log(
      ...
      "getTwitterTokens",
      "success",
      "Twitter tokens fetched successfully",
      {
        userId,
    ...
    log(
      ...
      "getTwitterTokens",
      "error",
      "Failed to fetch Twitter tokens",
      { userId, error: error.message },
    );
    throw error;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log(
        ...
        "postTwitterMessage",
        "error",
        "Failed to post tweet to Twitter",
        {
          userId,
          status: response.status,
          error: errorMessage,
