...
export function getUserRole() {
  ...
  try {
    ...
    const authStoreModule = import.meta.glob("../../stores/useAuthStore.js", {
      eager: true,
    });
    ...
    const role =
      authStore.currentUser?.role || authStore.simulate?.role || null;

    log("authUtilities.js", "getUserRole", "result", "User role retrieved", {
      role,
    });
    window.performanceTracker.step({
...
export async function getUserEmail() {
  ...
  try {
    ...
    log("authUtilities.js", "getUserEmail", "result", "User email retrieved", {
      hasEmail: !!email,
    });
    window.performanceTracker.step({
...
export function isUserRole(roleToCheck) {
  log("authUtilities.js", "isUserRole", "start", "Checking user role", {
    roleToCheck,
  });
  window.performanceTracker.step({
  ...
  try {
    ...
    log("authUtilities.js", "isUserRole", "result", "Role check complete", {
      roleToCheck,
      currentRole,
      hasRole,
    });
