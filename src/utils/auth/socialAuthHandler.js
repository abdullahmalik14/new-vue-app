...
function getActiveOAuthSessions() {
  if (!window.__twitterOAuthSessions) {
    console.log(
      "[Twitter OAuth] Creating new Map in window.__twitterOAuthSessions",
    );
    window.__twitterOAuthSessions = new Map();
  } else {
    console.log(
      "[Twitter OAuth] Using existing Map, size:",
      window.__twitterOAuthSessions.size,
    );
  }
...
export async function initiateTwitterLogin() {
  ...
  try {
    ...
    if (!clientId || !redirectUri) {
      throw new Error(
        "Missing Twitter OAuth configuration. Please set VITE_TWITTER_CLIENT_ID and VITE_TWITTER_REDIRECT_URI",
      );
    }
    ...
    log(
      ...
      "initiateTwitterLogin",
      "error",
      "Failed to initiate Twitter login",
      {
        error: error.message,
        stack: error.stack,
      },
...
export async function handleTwitterCallback(
  ...
  try {
    ...
    console.log(
      "[Twitter OAuth] Active sessions:",
      Array.from(activeOAuthSessions.keys()),
    );
    ...
    if (!session) {
      ...
      console.error("[Twitter OAuth] Session not found for state:", {
        ...
        allWindowKeys: Object.keys(window).filter(
          (k) => k.includes("twitter") || k.includes("oauth"),
        ),
      });
      throw new Error(
        `Invalid or expired OAuth state. State: ${state}. Available states: ${availableStates.join(", ") || "none"}`,
      );
    }
      ...
      try {
        ...
        if (errorJson.error_description) {
          ...
          if (errorJson.error_description.includes("authorization code")) {
            errorMessage +=
              ". The authorization code may have expired or been used. Please try logging in again.";
          }
        }
      } catch (e) {
    ...
    const email =
      ...
      profileResponse.data.confirmed_email ||
      null;
    ...
    const tokens = await authenticateOrSignUpTwitterUser(
      twitterUserId,
      email,
      intent,
    );
...
export async function exchangeTwitterCodeForTokens(code, state) {
  log(
    ...
    "start",
    "Exchange Twitter code for tokens",
    {
      hasCode: !!code,
      hasState: !!state,
    },
...
export async function getTwitterProfile(accessToken) {
  ...
  try {
    ...
    log(
      ...
      "getTwitterProfile",
      "profile-request",
      "Requesting profile from Twitter API",
      {},
    );
