...
async function authenticateWithCustomAuth(username) {
  ...
  try {
    ...
    console.log("[twitterCognitoHandler] Using Cognito app client:", {
      ...
      userPoolIdPrefix: userPoolId
        ? String(userPoolId).slice(0, 12)
        : "missing",
    ...
    if (response.AuthenticationResult) {
      ...
      log(
        ...
        "authenticateWithCustomAuth",
        "success",
        "Authentication successful (direct)",
        {
          username,
...
async function createTwitterUser(username, email, twitterUserId) {
  ...
  try {
    ...
    log(
      ...
      "createTwitterUser",
      "success",
      "Twitter user created successfully",
      {
        username,
    ...
    registerUser(username, {
      ...
      .then(() =>
        log(
          ...
          "createTwitterUser",
          "scylla-sync-success",
          "Synced Twitter user to ScyllaDB",
          { username },
        ),
      )
      .catch((err) =>
        log(
          ...
          "createTwitterUser",
          "scylla-sync-error",
          "Failed to sync Twitter user to ScyllaDB",
          { error: err.message },
        ),
      );
...
async function updateUserAttributesWithToken(accessToken, attributes) {
  log(
    "twitterCognitoHandler.js",
    "updateUserAttributesWithToken",
    "start",
    "Updating user attributes via token",
    {
 ...
 */
export async function authenticateOrSignUpTwitterUser(
  twitterUserId,
  email,
  intent = "login",
) {
  ...
  try {
    ...
    log(
      "twitterCognitoHandler.js",
      "authenticateOrSignUpTwitterUser",
      "check-scylla",
      "Checking ScyllaDB for existing user by Twitter ID",
      { twitterUserId },
    );
    ...
    if (existingUser && existingUser.user_id) {
      ...
      log(
        ...
        "authenticateOrSignUpTwitterUser",
        "scylla-found",
        "Found user in ScyllaDB. Logging in with Sub.",
      ...
      try {
        ...
        log(
          ...
          "authenticateOrSignUpTwitterUser",
          "sub-login-failed",
          "Login with Sub failed, will try other methods",
          { error: err.message },
        );
      }
    }
    ...
    if (email) {
      log(
        ...
        "authenticateOrSignUpTwitterUser",
        "check-email",
        "Checking ScyllaDB for existing email",
        { email },
      );
      ...
      if (emailUser) {
        ...
        if (emailUser.twitter_id === twitterUserId) {
          ...
          throw new Error(
            "Account exists with this email but is not linked to Twitter. Please log in with your email and password, then link Twitter in settings.",
          );
        }
      }
    }
    ...
    const usernameToCreate =
      email && email.trim() ? email.trim() : syntheticUsername;

    log(
      "twitterCognitoHandler.js",
      "authenticateOrSignUpTwitterUser",
      "create-user",
      "Creating new Twitter user",
    ...
    log(
      ...
      "authenticateOrSignUpTwitterUser",
      "error",
      "Twitter authentication/signup failed",
      {
        twitterUserId,
        intent,
        error: error.message,
