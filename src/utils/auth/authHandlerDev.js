...
function loadMockUserFromRecord(record) {
  ...
  mockUser.raw["custom:onboardingPassed"] = record.onboardingPassed
    ? "true"
    : "false";
}
...
function signJwtHS256(payload, options = {}) {
  log("authHandlerDev.js", "signJwtHS256", "start", "Creating mock JWT token", {
    hasPayload: !!payload,
  });
  ...
  log("authHandlerDev.js", "signJwtHS256", "success", "Mock JWT created", {
...
async function authenticateDevUser(email, password) {
  ...
  try {
    ...
    log(
      ...
      "success",
      "Dev login complete",
      { email, tokenLength: idToken.length },
    );
    window.performanceTracker.step({
...
async function registerDevUser(email, password, attributes = {}) {
  ...
  try {
    ...
    log(
      ...
      "registerDevUser",
      "success",
      "Dev registration successful",
      { email: normalizedEmail, userSub },
    );
    window.performanceTracker.step({
...
async function handleDevCallback() {
  ...
  try {
    ...
    log(
      ...
      "handleDevCallback",
      "no-tokens",
      "No pending tokens, simulating callback",
      {},
    );
    window.performanceTracker.step({
  ...
  try {
      ...
      try {
        ...
        if (exp && exp - now < 60) {
          ...
          log(
            ...
            "restoreDevSession",
            "expired",
            "Token expiring or expired, attempting refresh",
            { exp, now },
          );
        }
      } catch (e) {
    ...
    if (pendingTokens) {
      log(
        ...
        "restoreDevSession",
        "pending-tokens",
        "Using pending tokens for restoration",
        {},
      );
...
async function updateDevUserAttributes(attributes) {
  log(
    "authHandlerDev.js",
    "updateDevUserAttributes",
    "start",
    "Updating dev user attributes",
...
async function confirmDevSignup(email, code) {
  ...
  try {
    ...
    log("authHandlerDev.js", "confirmDevSignup", "success", "Email confirmed", {
      email,
    });
    window.performanceTracker.step({
...
async function resendDevConfirmationCode(email) {
  log(
    "authHandlerDev.js",
    "resendDevConfirmationCode",
    "start",
    "Resending dev confirmation code",
    { email },
  );
  window.performanceTracker.step({
...
function setMockUserState(state = {}) {
  ...
  if (state.onboardingPassed !== undefined) {
    ...
    mockUser.raw["custom:onboardingPassed"] = state.onboardingPassed
      ? "true"
      : "false";
  }
...
async function validateDevToken(token) {
  ...
  try {
    ...
    if (exp && Date.now() / 1000 > exp) {
      log("authHandlerDev.js", "validateDevToken", "expired", "Token expired", {
        exp,
      });
      window.performanceTracker.step({
    ...
    if (payload.sub !== mockUser.raw.sub) {
      log(
        ...
        "validateDevToken",
        "invalid_sub",
        "Token sub claim does not match mock user sub",
        { tokenSub: payload.sub, mockUserSub: mockUser.raw.sub },
      );
      window.performanceTracker.step({
    ...
    log("authHandlerDev.js", "validateDevToken", "valid", "Token valid", {
      token,
    });
    window.performanceTracker.step({
...
function mockUserRole(role) {
  log("authHandlerDev.js", "mockUserRole", "start", "Mocking user role", {
    role,
  });
  window.performanceTracker.step({
...
if (typeof window !== "undefined") {
  ...
  log(
    ...
    "window-api",
    "expose",
    "Dev auth utilities exposed on window.APP",
    {
      commands: [
