...
// We use calc(percentage% + offset) to adjust for thumb width if necessary
// But simple percentage usually works well enough with translateX(-50%)
...
// Methods
const handleBack = () => {
    // Check if engine exists and has navigation
    if (props.engine && props.engine.goToStep) {
        props.engine.goToStep(1);
    } else {
        console.warn('Engine navigation not available');
    }
};
...
<template>
    <div class="flex flex-col h-full w-full relative">
        ...
        <div class="flex flex-col gap-4 px-2 pt-4 pb-6 md:px-4 md:pt-4 lg:px-6 lg:pt-6 xl:p-6">
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <!-- Alert Component -->
                    <div v-if="showMaxLimitAlert" class="w-full relative">
                        <NotificationCard variant="limit-exceeded" :showIcon="false" 344054 :closable="false"
                            class="w-full">
                            <template #default>
                                <div class="relative w-full flex gap-4">
                                    <div class="relative">
                                        <img src="https://i.ibb.co.com/Y49kymq4/alert-icon.webp" alt="alert-icon"
                                            class="h-10" />
                                    </div>
                                    <div class="flex flex-col w-full">
                                        <p class="text-sm font-semibold text-[#ff4405]">Input value exceeds maximum
                                            allowance (14,000 tokens)</p>
                                        <div class="flex justify-end items-end w-full">
                                            <button @click="setToMax"
                                                class="flex items-center gap-1 p-0 bg-transparent outline-none border-none cursor-pointer">
                                                <span class="text-xs leading-normal font-medium text-[#ff4405]">Set to
                                                    Max</span>
                                                <img src="https://i.ibb.co.com/8njxTqWK/chevron-left.webp"
                                                    alt="chevron-left"
        ...
        <div class="flex flex-col mt-auto bg-white/50 dark:bg-[#181a1b80] pb-[3.25rem] sm:pb-0 flex-1">
            ...
            <!-- button -->
            <div class="flex justify-end items-end mt-auto fixed sm:relative bottom-0 right-0 z-[100000] sm:z-auto">
