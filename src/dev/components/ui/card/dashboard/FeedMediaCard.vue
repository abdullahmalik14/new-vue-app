    ...
    <div class="flex flex-col justify-end gap-2 p-4 z-[1] flex-grow">
      ...
      <div class="flex items-end gap-4">
        ...
        <div class="flex items-center gap-2">
          <img
            :src="resolvedLikeIcon"
            :alt="t('profile.alt.notLiked')"
        ...
        <div class="flex items-center gap-2">
          <img
            :src="resolvedCommentIcon"
            :alt="t('profile.alt.messageText')"
            class="w-6 h-6 [filter:brightness(0)_saturate(100%)_invert(99%)_sepia(36%)_saturate(108%)_hue-rotate(297deg)_brightness(96%)_contrast(88%)] group-hover:[filter:brightness(0)_saturate(100%)_invert(70%)_sepia(47%)_saturate(1521%)_hue-rotate(87deg)_brightness(100%)_contrast(98%)]"
          />
          <span
            class="text-xs leading-normal tracking-[0.005rem] text-[#E7E5E4] align-middle group-hover:text-[#07F468]"
...
const props = defineProps({
  ...
  likeIcon: { type: String, default: "" },
  commentIcon: { type: String, default: "" },
});

const { t } = useI18n();
const { url: likeIconDefault } = useAssetUrl("profile.card.notLiked");
const { url: commentIconDefault } = useAssetUrl("profile.card.messageText");

const resolvedLikeIcon = computed(() => props.likeIcon || likeIconDefault.value);
const resolvedCommentIcon = computed(() => props.commentIcon || commentIconDefault.value);
