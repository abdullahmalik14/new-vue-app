...
import PurchaseFlowSubscriptionOrderPopup from "@/dev/components/ui/popups/profile/PurchaseFlowSubscriptionOrderPopup.vue";
...
const guestPurchaseFlowPopupConfig = computed(() => {
  if (isMobile.value) {
    ...
    return {
      ...
      speed: "250ms",
      effect: "ease-in-out",
      shouldShowOverlay: true, // Desktop pe overlay true rakhein ya false apki marzi
      shouldCloseOnOutsideClick: true,
      shouldLockBodyScroll: true,
      shouldCloseOnEscape: true,
...
<template>
  <BasePopup :modelValue="modelValue" @update:modelValue="(val) => emit('update:modelValue', val)"
    :popupConfig="guestPurchaseFlowPopupConfig">
    <div
