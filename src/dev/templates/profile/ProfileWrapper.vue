        ...
        <div class="tab-content active relative flex-1 min-h-[100svh] lg:bg-black/50" id="feed-content">
          <div
            ...
            <section class="relative flex flex-col gap-2 lg:flex-row lg:gap-0 lg:w-[calc(100%-9rem)] lg:mx-auto">
              <div>
                <BaseHeading text="MARCH" tag="h2" theme="profileFeed" />
                <BaseHeading text="2024" tag="h2" theme="profileFeed" />
              </div>
            ...
            <section class="relative flex flex-col gap-2 lg:flex-row lg:gap-0 lg:w-[calc(100%-9rem)] lg:mx-auto">
              <div>
                <BaseHeading text="FEBRUARY" tag="h2" theme="profileFeed" />
                <BaseHeading text="2024" tag="h2" theme="profileFeed" />
              </div>
        ...
        <div class="tab-content hidden relative flex-1 min-h-[65.25rem] pt-20 md:pt-32" id="media-content">
          <section class="mb-8">
            ...
            <div class="mx-2 overflow-visible md:mx-4 xl:mx-12">
              <ul data-media-list-container
                class="grid gap-2 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 xl:gap-4">
                <MediaCardV2 v-for="(entry, index) in mediaGridCards" :key="index" :media="entry.card"
                  aspect-class="aspect-[2/1.1522]" class="cursor-pointer" @click="openMediaPopup(entry.source)" />
              </ul>
            </div>
          </section>
...
import ProfileHeader from "@/dev/components/profile/ProfileHeader.vue";
import ProfileMeta from "@/dev/components/profile/ProfileMeta.vue";
import ProfileSubscribeCta from "@/dev/components/profile/ProfileSubscribeCta.vue";
import ProfileSharing from "@/dev/components/profile/ProfileSharing.vue";
import ProfileRightButtons from "@/dev/components/profile/ProfileRightButtons.vue";
import BaseParagraph from "@/components/ui/typography/BaseParagraph.vue";
import ProfileDescription from "@/dev/components/profile/ProfileDescription.vue";
import ProfileSharingResponsive from "@/dev/components/profile/ProfileSharingResponsive.vue";
import ProfileMetaResponsive from "@/dev/components/profile/ProfileMetaResponsive.vue";
import ProfileHeaderUserResponsive from "@/dev/components/profile/ProfileHeaderUserResponsive.vue";
import ProfileHeaderUserDesktop from "@/dev/components/profile/ProfileHeaderUserDesktop.vue";
import ProfileHeaderOptionButtons from "@/dev/components/profile/ProfileHeaderOptionButtons.vue";
import ProfileMediaTabHeading from "@/dev/components/profile/ProfileMediaTabHeading.vue";
import ProfileMediaDetailsPopup from "@/components/ui/popups/ProfileMediaDetailsPopup.vue";
import ProfileMerchPopup from "@/dev/components/ui/popups/profile/ProfileMerchPopup.vue";
...
// Map raw media items to the reusable MediaCardV2 shape (keeps original item
// for the details popup, which reads media.video / media.title).
const mediaGridCards = computed(() =>
  myMediaData.value.map((item) => ({
    source: item,
    card: {
      type: "video",
      poster: item.image,
      ...
      title: item.title,
      showTag: true,
      tagText: t("profile.labels.subscribeOrBuy"),
      tagColor: "pink",
    },
  })),
);
...
// Hero slider ki .lazyload images ko manually load karao. Strict lazysizes
// (loadMode 1 / expFactor 0) Splide ke andar 0-height/transform slides ko skip
// kar deta hai, is liye above-the-fold hero bg kabhi unveil nahi hota.
const forceUnveilHeroImages = () => {
  const heroImages = document.querySelectorAll(
    "#splide01 img.lazyload[data-src]"
  );
...
onMounted(() => {
  ...
  if (typeof Splide !== "undefined") {
    ...
    setTimeout(() => {
      syncAllSliders(0);
      splide03.refresh();
      // Hero above-the-fold slides hain, magar strict lazysizes (loadMode 1,
      // expFactor 0) inhe Splide ke transform/0-height ki wajah se skip kar deta
      // hai. Is liye in .lazyload images ko manually unveil kar rahe hain.
      forceUnveilHeroImages();
    }, 100);
  }
