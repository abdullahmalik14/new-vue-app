    ...
    <div
      ...
      <div class="w-80 border-r border-zinc-200 flex flex-col bg-gray-50/50 shrink-0">
        <div class="p-5 border-b border-zinc-200 bg-white flex items-center justify-between shadow-sm z-10">
          ...
          <div
            class="w-8 h-8 rounded-full bg-rose-100 flex items-center justify-center text-rose-700 font-bold shrink-0">
            {{ displayInitials }}
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto w-full">
          <div v-for="contact in contactList" :key="contact.id" @click="selectChat(contact.id)" :class="[
            ...
            selectedChatId === contact.id ? 'bg-rose-50/80 border-l-4 border-l-rose-500' : 'hover:bg-zinc-100 border-l-4 border-l-transparent'
          ]">
          ...
          <FlexChat class="h-full flex flex-col w-full absolute inset-0" :messages="activeMessages"
              ...
              <div class="w-full">
                <div class="flex items-center justify-between w-full">
                    ...
                    <div class="flex flex-col">
                      <div class="text-gray-900 font-bold text-[15px] flex items-center gap-2">
                        ...
                        <span v-if="selectedChat.type === 'private'"
                          class="px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 text-[10px] uppercase font-bold tracking-wider">DM</span>
                      </div>
                  ...
                  <div class="flex items-center gap-4 text-zinc-400">
                    <svg class="w-5 h-5 cursor-pointer hover:text-rose-600 transition-colors" fill="none"
            ...
            <template #message.avatar.me="{ message }">
              <img v-if="message.time" :src="getUserAvatarUrl(message.senderId)"
                class="w-6 h-6 rounded-full object-cover shadow-sm bg-rose-50 border border-rose-100" />
            ...
            <template #compose>
              <div class="flex items-center gap-3 my-1 w-full relative">
                ...
                <div class="flex gap-4 text-zinc-400 justify-end w-auto whitespace-nowrap items-center shrink-0">
                  <svg class="w-5 h-5 cursor-pointer hover:text-rose-500 transition-colors" fill="none"
                    stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  ...
                  <button @click="handleSend"
                    class="ml-2 bg-rose-500 hover:bg-rose-600 text-white rounded-full p-2 shadow-md transition-transform active:scale-95">
...
import FlexChat from '@/dev/components/chat/FlexChat.vue'
...
// userId extraction (same logic as creator side):
// - Real Cognito sub (UUID): use as-is ✅
// - Mock sub 'mock-user-XXXXX': strip prefix → numeric (AWS accepts) ✅
...
const currentUserRole = computed(() => authStore.userRole || 'fan')

// --- CHAT PARTICIPANTS CONFIG ---
// DO NOT change these hardcoded subs — they are for socket routing only
// Creator : '97c45ac8-70e1-7093-2a13-c492458b486c' (real UUID)
const CREATOR_USER_ID = '97c45ac8-70e1-7093-2a13-c492458b486c'

// STABLE email prefix — use karo chatId ke liye (DB mein stable rehta hai across logins)
// abfan@test.com → 'abfan' | fan2@test.com → 'fan2' — same har login par!
const stableEmailId = computed(() => {
  const email = String(authStore.currentUser?.email || '').trim().toLowerCase()
  return email ? email.split('@')[0] : currentUserId.value
})

// STABLE socket routing IDs — numeric use karo (AWS validation bypass)
...
const GROUP_CHAT_MEMBERS = [CREATOR_USER_ID, FAN1_ID, FAN2_ID, FAN3_ID]

// Map email to exact acceptable numeric socket ID
const stableSocketId = computed(() => {
  const map = {
    'abfan': FAN1_ID,
    'fan2': FAN2_ID,
    'fan3': FAN3_ID
  }
  return map[stableEmailId.value] || currentUserId.value
})
  ...
  const list = [
    {
      ...
      members: GROUP_CHAT_MEMBERS.value,
      avatar: 'https://ui-avatars.com/api/?name=VVIP&background=0f172a&color=fff&rounded=true'
    }
  ]

  // DM chatId = 'dm-{email-prefix}' (STABLE across logins!)
  // abfan@test.com → 'dm-abfan' | fan2@test.com → 'dm-fan2'
  // Yeh chatId creators ke DM id se match karta hai: dm-abfan, dm-fan2
  list.push({
    id: `dm-${stableEmailId.value}`,   // STABLE: 'dm-abfan' (changes nahi har login par)
    type: 'private',
    name: 'dev (Creator)',
    recipientId: CREATOR_USER_ID,       // socket routing ke liye (AWS WebSocket to)
    role: 'Creator',
    avatar: 'https://ui-avatars.com/api/?name=DEV&background=22c55e&color=fff&rounded=true'
  })
...
// ID → Display name mapping (PEHLE declare karo)
// Fan component mein sirf CREATOR_USER_ID defined hai — baki email prefix use karo
const getUserDisplayName = (userId) => {
  if (!userId) return 'Unknown'
  const map = {
    [CREATOR_USER_ID]: 'dev (Creator)',   // Creator UUID
    // Email prefix fallbacks (stable across logins — always work)
    'dev': 'dev (Creator)',
    'abfan': 'abfan',
    'fan2': 'fan2',
    'fan3': 'fan3',
...
// getUserAvatarUrl — getUserDisplayName ke BAAD declare karo
...
// We preserve the core theme styles exactly as instructed
const chatTheme = {
  ...
  myBubble: 'text-white text-[15px] font-medium max-w-[85%] min-w-16 min-h-10 px-4 py-2 bg-rose-500 rounded-tl-2xl rounded-tr-2xl rounded-br-2xl shadow-sm inline-flex justify-center items-center gap-2.5',
  otherBubble: 'text-gray-800 text-[15px] font-medium max-w-[85%] min-w-16 min-h-10 px-4 py-2 bg-white border border-gray-100 rounded-tl-2xl rounded-tr-2xl rounded-bl-2xl shadow-sm inline-flex justify-center items-center gap-2.5',
  systemBubble: 'w-full',
...
const handleTyping = () => {
  ...
  const typingRecipients = chat.type === 'group'
    ? GROUP_CHAT_MEMBERS.filter(uid => uid !== stableSocketId.value)
    : [chat.recipientId || CREATOR_USER_ID]
  ...
  typingRecipients.forEach(recipientId => {
    SocketHandler.sendSocketMessage({
      ...
      payload: {
        ...
        to: recipientId,
        senderId: stableSocketId.value,
        senderName: stableEmailId.value // stable prefix as display name
      }
...
const receiveTyping = (chatId, userId, senderName) => {
  if (!userId || userId === stableSocketId.value) return

  if (!activeTypingSets.value[chatId]) {
    activeTypingSets.value[chatId] = new Set()
  }

  // Store userId so it correctly matches payload.senderId on message arrival
...
const fetchMoreActiveMessages = async () => {
  ...
  if (res.ok) {
    if (res.data.items && res.data.items.length > 0) {
      chatStore.syncChatHistoryPage({ chatId: cid, items: res.data.items, pagingState: res.data.pagingState })
    }
...
// Mount par SAARE chats DB se fetch karo
const fetchAllChatsFromDB = async () => {
  ...
  for (const chatId of allChatIds) {
    ...
    if (res.ok && res.data.items && res.data.items.length > 0) {
      chatStore.syncChatHistoryPage({ chatId, items: res.data.items, pagingState: res.data.pagingState })
      console.log(`[Fan] ✅ Fetched ${res.data.items.length} msgs for ${chatId}`)
    }
  }
}
...
const handleSend = async () => {
  ...
  const messageObj = {
    ...
    senderName: getUserDisplayName(currentUserId.value),
    text,
    type: 'text',
    time: timeStr
  }
  ...
  // For groups: loop and send one socket message per recipient.
  // For DMs: send to the creator's actual userId.
  const chat = selectedChat.value

  // Filter out SELF using stable numeric ID
  ...
  const basePayload = {
    id: tempMsgId,
    chatId: targetChatId,
    senderId: stableSocketId.value,         // numeric acceptable ID
    senderName: stableEmailId.value,        // display ke liye stable naam
    text,
    type: 'text',
    time: timeStr
  }
  ...
  socketRecipients.forEach(recipientId => {
    ...
    }).catch(e => console.error(`[Fan] Failed to send socket msg to ${recipientId}:`, e))
  })
}

// ---------------- SOCKET LISTENERS ----------------
const handleSocketIncoming = (payload) => {
  console.log("[DashboardChatsFan] 📥 Socket Incoming:", payload)
  if (!payload) return
  ...
  // FILTER: Ignore messages we sent ourselves (echo prevention)
  // Match against our numeric socket ID
  if (payload.senderId === stableSocketId.value) {
    console.log("[DashboardChatsFan] ⏭️ Skipping own echo message:", payload.id)
    return
  }

  // Clear typing for this sender on message receive
  if (payload.senderId && activeTypingSets.value[payload.chatId]) {
    // BUG FIX: Delete by senderId, because we store userId in activeTypingSets now!
  ...
  const constructedMsg = {
    ...
    senderName: payload.senderName || getUserDisplayName(payload.senderId),
    text: payload.text,
...
onMounted(async () => {
  ...
  // Saare chats DB se fetch karo
  await fetchAllChatsFromDB()
  activeLoading.value = false

  try {
    // CRITICAL FIX: Connect varying logins to SAME consistent NUMERIC ID
    // Fan1 always connects as '1775053005043'. This fixes AWS validation errors! ✔️
    SocketHandler.identifyCurrentUser(stableSocketId.value)
    ...
    console.log(`[DashboardChatsFan] WebSocket active as: ${stableSocketId.value} (from email: ${stableEmailId.value})`)
  } catch (err) {
