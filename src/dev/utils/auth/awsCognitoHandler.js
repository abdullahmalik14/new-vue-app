...
import { registerUser } from "@/infrastructure/backend/scyllaDbClient";
/**
...
export async function registerNewUser(email, password, attributes = {}) {
  log(
    ...
    { email, attributeKeys: Object.keys(attributes) },
  );
  window.performanceTracker.step({
  ...
  try {
    ...
    if (!("custom:joinedWithSocial" in attributesWithFlag)) {
      ...
      attributesWithFlag["custom:joinedWithSocial"] = isSocialSignup
        ? "true"
        : "false";
    }
...
export async function resendConfirmationCode(email) {
  log(
    ...
    "start",
    "Begin resending confirmation code",
    { email },
  );
  window.performanceTracker.step({
...
export async function changeUserPassword(currentPassword, newPassword) {
  log(
    ...
    { hasCurrentPassword: !!currentPassword, hasNewPassword: !!newPassword },
  );
  window.performanceTracker.step({
...
export async function getUserAttributes() {
  ...
  try {
    ...
    log(
      ...
      "getUserAttributes",
      "fallback-error",
      "Failed to get attributes via token",
      { error: error.message },
    );
    return {};
  }
...
export async function adminSetUserPassword(newPassword) {
  log(
    ...
    "start",
    "Setting user password via admin (Client-side implementation)",
    {},
  );
  ...
  if (!accessKeyId || !secretAccessKey) {
    throw new Error(
      "Missing AWS Credentials (VITE_AWS_ACCESS_KEY_ID / VITE_AWS_SECRET_ACCESS_KEY) for Admin Action",
    );
  }
  ...
  try {
    ...
    log(
      ...
      "adminSetUserPassword",
      "sdk-attempt",
      "Sending AdminSetUserPassword command",
      { username },
    );
    ...
    log(
      ...
      "adminSetUserPassword",
      "sdk-success",
      "AdminSetUserPassword successful",
      {},
    );
...
export async function confirmPasswordReset(email, code, newPassword) {
  log(
    ...
    "start",
    "Begin password reset confirmation",
    { email, hasCode: !!code, hasNewPassword: !!newPassword },
  );
  window.performanceTracker.step({
    ...
    if (!currentUser) {
      ...
      if (!accessToken) {
        log(
          ...
          "updateUserProfile",
          "no-user",
          "No authenticated user found (no current user and no access token)",
          {},
        );
        throw new Error("Not authenticated");
      }
      ...
      log(
        ...
        "Updating attributes via UpdateUserAttributes (token-only session)",
        {
          attributeKeys: Object.keys(attributes),
      ...
      log(
        ...
        "updateUserProfile",
        "success",
        "Profile update complete (token-only session)",
        {},
      );
      return "OK";
    }
  ...
  try {
    ...
    if (!currentUser) {
      ...
      if (!refreshToken) {
        if (idToken && accessToken) {
          log(
            ...
            "restoreUserSession",
            "fallback-stored",
            "Returning stored tokens (no current user)",
            {
              hasIdToken: true,
              hasAccessToken: true,
            },
        ...
        log(
          ...
          "restoreUserSession",
          "no-user",
          "No current user found and no refresh token available",
          {},
        );
      ...
      const newAccessToken =
        resp.AuthenticationResult?.AccessToken || accessToken;
      ...
      log(
        ...
        "restoreUserSession",
        "fallback-refreshed",
        "Session refreshed via REFRESH_TOKEN_AUTH (token-only session)",
        {
...
export function getPendingTokens() {
  log(
    ...
    "getPendingTokens",
    "start",
    "Getting pending tokens",
    { hasPendingTokens: !!pendingTokens },
  );
  window.performanceTracker.step({
  ...
  log(
    ...
    "getPendingTokens",
    "success",
    "Pending tokens retrieved and cleared",
    { hadTokens: !!tokens },
  );
  window.performanceTracker.step({
