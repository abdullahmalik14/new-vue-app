# Folder structure audit — asset code

**Scope:** All asset-related code in `new-vue-app-main/src/` (per `notes.md` responsibility rules)  
**Reference:** `notes.md`  
**Code index:** [asset-code-index.md](./asset-code-index.md)  
**Docs:** [docs/ASSET_PLAN.md](./docs/ASSET_PLAN.md) — master plan (developer + AI)  
**Audit type:** File locations and structure only

**Target layers:**

- `systems/assets/` — asset library, preloader, scanner, policy, map source, route prefetch
- `composables/` — `useAssetPrefetch.js`, `useRoutePrefetch.js`
- `config/` — `assetMap.json` and related JSON
- `assets/` — static CSS, images, icons, fonts only
- `stores/usePreloadStore.js` — preload state
...
**Why it is an issue:** Shared asset preload catalog — asset config, not route config. `notes.md` limits `router/` to Vue Router entry and route config only. Imported by `routeConfigLoader.js`, `jsonConfigValidator.js`, and `resolveSharedComponentAssets.js`.

**Suggested fix:** Move to `config/sharedAssetPreloads.json`. Update all three imports.
...
**Why it is an issue:** Standalone `preloadIcons()` using raw `Image()` preloading. Duplicates `systems/assets/assetPreloader.js`. Still in `utils/` after migration.
...
**Why it is an issue:** `notes.md` defines `src/assets/` as CSS, images, icons, and fonts only — not JS data modules. `menuItems.js` resolves asset flags (correct pattern, wrong folder). `settingConfig.js` uses hardcoded ImgBB URLs instead of `assetMap.json` flags.

**Suggested fix:** Move data files to `config/` (e.g. `config/menuItems.js`, `config/settingConfig.js`). Replace hardcoded icon URLs in `settingConfig.js` with asset flags resolved via `getAssetUrl`.
...
**Why it is an issue:** `notes.md` expects `assets/images/`, `assets/icons/`, and `assets/fonts/`. Only CSS files and a `data/` subfolder exist.

**Suggested fix:** Create the missing folders when static files are added, or update `notes.md` if all images are served via `assetMap.json` CDN URLs only.
...
**Why it is an issue:** `notes.md` lists only `assetMap.json` and `countries.json` under `config/`. Section-scoped maps are loaded via `sectionAssetMapSource.js` glob pattern — undocumented in the target structure.

**Suggested fix:** Add `assetMap.*.json` section maps to `notes.md` as intentional, or consolidate into fewer config files.
...
**Why it is an issue:** Page manually builds `AssetHandler` config and instantiates `AssetHandler` directly instead of using `createAssetHandler` from the assets system.

**Suggested fix:** Use `createAssetHandler` from `assetHandlerFactory.js`, or extract `loadDashboardCreatorAssets()` into `systems/assets/`.

---

## Issue 22

**Location:** Six auth views (`AuthLogIn.vue`, `AuthSignUp.vue`, `AuthResetPassword.vue`, `AuthLostPassword.vue`, `AuthConfirmEmail.vue`, `AuthSignUpOnboarding.vue`)

**Why it is an issue:** Each view defines its own `assetsConfig` array and calls `createAssetHandler` inline. Duplicated asset-loading setup across templates.

**Suggested fix:** Extract shared auth asset config to `systems/assets/` (e.g. `authAssetConfig.js`). Import one config and factory call from all auth views.
...
**Why it is an issue:** Tests still import `src/utils/assets/*` and `src/utils/route/routeAssetPrefetch.js`. Those paths no longer exist after migration to `systems/assets/` and `systems/routing/`.

**Suggested fix:** Bulk-update test imports to `src/systems/assets/*` and correct routing paths. Re-run test suite to confirm.

---

## Issue 24

**Location:** `composables/useAssetUrl.js` (present but not in `notes.md`)

**Why it is an issue:** `notes.md` lists `useAssetPrefetch.js` but not `useAssetUrl.js`. The composable exists and correctly wraps `getAssetUrl` from the assets system.

**Suggested fix:** Add `useAssetUrl.js` to `notes.md` composables list, or merge into `useAssetPrefetch` if they serve the same concern.

---

*End of audit — 24 issues.*
