# Section master plan

**Canonical doc:** [docs/SECTION_PLAN.md](./docs/SECTION_PLAN.md)  
**Docs hub:** [docs/README.md](./docs/README.md)

---

## For developers

- [DEVELOPER_GUIDE.md](./docs/DEVELOPER_GUIDE.md) — how the system works
- Priority work: **P0** test imports → **P1** structure → **P2** naming → **P3** docs

## For AI

- [AI_GUIDE.md](./docs/AI_GUIDE.md) — rules and boundaries
- Read audits before large refactors: [sections-code-audit.md](./sections-code-audit.md), [section-code-index.md](./section-code-index.md)

## Audits & indexes

| Item | Link |
|------|------|
| Structure audit | [folder-structure-audit-systems-sections.md](./folder-structure-audit-systems-sections.md) |
| Code audit | [sections-code-audit.md](./sections-code-audit.md) |
| Loose code scan | [loose-section-code-scan.md](./loose-section-code-scan.md) |
| Code index | [section-code-index.md](./section-code-index.md) |
| Naming audit | [sections-naming-audit.md](./sections-naming-audit.md) |
| Test plan (Vitest) | [section-test-plan.md](./section-test-plan.md) |
| **Change log** | [sections-cleanup-changelog.md](./sections-cleanup-changelog.md) |

*Full plan detail lives in `docs/SECTION_PLAN.md` — update that file when priorities change.*
